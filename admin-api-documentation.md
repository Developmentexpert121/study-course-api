...
This document lists the API endpoints accessible to Admin users in the Study Course API. Admins can manage their own courses, view their dashboard stats, and access user-related information.

Base URL: `http://localhost:5000` (adjust as per your environment)

## Authentication
All endpoints require authentication with an Admin role. Use the `/user/login` endpoint to get a token, then include it in the Authorization header as `Bearer <token>`.
...
- Admin roles have permissions to manage their own courses and view related data.
