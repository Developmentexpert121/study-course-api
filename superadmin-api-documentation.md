...
This document lists the API endpoints accessible to Super Admin users in the Study Course API. Super Admins have full access to all courses, users, and system-wide data across all admins.

Base URL: `http://localhost:5000` (adjust as per your environment)

## Authentication
All endpoints require authentication with a Super-Admin role. Use the `/user/login` endpoint to get a token, then include it in the Authorization header as `Bearer <token>`.
...
#### GET `/user/getlogs` - Get admin activity logs (Super Admin)
**Response:**
```json
{
  "success": true,
...
#### GET `/user/` - Get all users (Super Admin)
**Query Parameters:**
- `page` (optional): Page number (default: 1)
- `limit` (optional): Items per page (default: 10)
...
#### GET `/user/stats` - Get user stats (Super Admin)
**Response:**
```json
{
  "success": true,
...
#### GET `/user/summary` - Get dashboard summary (Super Admin)
**Response:**
```json
{
  "success": true,
...
#### GET `/user/get-all-details` - Get all user details (Super Admin)
**Query Parameters:**
- `page` (optional): Page number (default: 1)
- `limit` (optional): Items per page (default: 10)
...
#### GET `/user/details/:id` - Get user details by ID (Super Admin)
**Path Parameters:**
- `id`: User ID
...
#### GET `/course/admin/all-courses` - Get all courses for admin (Super Admin)
**Query Parameters:**
- `active` (optional): Filter by active status
- `status` (optional): Filter by status ('active', 'inactive', 'draft')
- `search` (optional): Search term
- `include_chapters` (optional): Include chapters ('true' or 'false')
- `page` (optional): Page number (default: 1)
- `limit` (optional): Items per page (default: 10)
- `category` (optional): Filter by category
- `sort` (optional): Sort option ('newest', 'oldest', 'popular', 'ratings', 'title', 'price', 'creator')
...
#### PUT `/course/:id/status` - Toggle course status (Super Admin)
**Path Parameters:**
- `id`: Course ID
...
#### DELETE `/course/:id` - Delete course (Super Admin)
**Path Parameters:**
- `id`: Course ID
...
#### POST `/course/create-course` - Create course (Super Admin)
**Request Body:**
```json
{
...
- **POST** `/chapter/` - Create chapter (Super Admin)
- **DELETE** `/chapter/:id` - Delete chapter (Super Admin)
...
- **GET** `/comment/:courseId` - Get comments by course (Super Admin)
...
- **GET** `/certificate/admin/courses/:courseId/enrolled-users` - Get enrolled users with progress (Super Admin)
- **POST** `/certificate/admin/courses/:courseId/users/:userId/generate-certificate` - Generate certificate (Super Admin)
- **POST** `/certificate/admin/certificates/:certificateId/send-email` - Send certificate email (Super Admin)
- **POST** `/certificate/admin/certificates/:certificateId/revoke` - Revoke certificate (Super Admin)
- **POST** `/certificate/admin/certificates/:certificateId/reinstate` - Reinstate certificate (Super Admin)
- **POST** `/certificate/admin/certificates/bulk-actions` - Bulk certificate actions (Super Admin)
...
- **GET** `/certificate/stats/overview` - Get certificate stats overview (Super Admin)
...
- **GET** `/rating/` - Get all ratings (Super Admin)
- **GET** `/rating/course/:courseId` - Get ratings by course (Super Admin)
...
- **PATCH** `/rating/:ratingId/hide` - Hide rating (Super Admin)
- **PATCH** `/rating/:ratingId/unhide` - Unhide rating (Super Admin)
- **PATCH** `/rating/:ratingId/hide-review` - Hide review (Super Admin)
- **PATCH** `/rating/:ratingId/unhide-review` - Unhide review (Super Admin)
- **DELETE** `/rating/user/:ratingId` - Delete user rating
- **DELETE** `/rating/:ratingId` - Delete rating (Super Admin)
...
- **POST** `/mcq/create-mcq` - Create MCQ (Super Admin)
- **PUT** `/mcq/:id` - Update MCQ (Super Admin)
- **PUT** `/mcq/:id/status` - Toggle MCQ status (Super Admin)
- **DELETE** `/mcq/:id` - Delete MCQ (Super Admin)
...
- Super Admins have full access to all courses, users, and system-wide data across all admins.
