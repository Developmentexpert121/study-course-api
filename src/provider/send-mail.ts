...
// ✅ Production-ready transporter with better error handling
...
if (transporter) {
  transporter.verify((error, success) => {
    if (error) {
      ...
      console.log('1. Verify EMAIL_USER is set:', process.env.EMAIL_USER ? '✅ Set' : '❌ Missing');
      console.log('2. Verify EMAIL_PASSWORD is set:', process.env.EMAIL_PASSWORD ? '✅ Set' : '❌ Missing');
      console.log('3. Ensure you are using Gmail App Password, not regular password');
...
const sendEmailWithRetry = async (emailFunction, ...args) => {
  const MAX_RETRIES = 3;
    ...
    transporter.sendMail(message, (err, info) => {
      if (err) {
        ...
        if (err.code === 'EAUTH') {
          ...
          console.error('⚠️ Connection refused - Gmail may be blocking your server');
        }
...
const sendWelcomeEmail = async (email: string, username?: string): Promise<boolean> => {
  // (Keep your existing welcome email HTML)
  const html = `...`; // Your existing HTML
...
const sendApprovalEmail = async (email: string, username: string): Promise<boolean> => {
  // (Keep your existing approval email HTML)
  const html = `...`; // Your existing HTML
...
const sendBulkEmail = async (req, res) => {
  try {
    ...
    const Email = req.app.get('models').Email; // Adjust based on your setup
    const subscribers = await Email.findAll({
      attributes: ['id', 'email']
    });
      ...
      const batchPromises = batch.map(async (subscriber) => {
        try {
          const emailSent = await sendEmailWithRetry(
            sendEmail,
            htmlContent || generateEmailTemplate(subject, message),
            subscriber.email,
            subject
          );
...
export { 
  ...
  sendBulkEmail,
  generateEmailTemplate,
  sendEmailWithRetry // Export the retry utility
};