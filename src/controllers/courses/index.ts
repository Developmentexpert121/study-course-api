...
export const getAllCoursesForAdmin = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || (userRole !== 'Teacher' && userRole !== 'Super-Admin')) {
      return res.status(403).sendError(res, "Teacher access required");
    }
...
export const createCourse = async (req: Request, res: Response) => {
  try {
    ...
    if (mode && !validModes.includes(mode)) {
      return res.sendError(res, "Mode must be one of: online, offline");
    }
  ...
  try {
    ...
    if (view_type === 'Teacher') {
      ...
      if (userRole === 'Super-Admin') {
        ...
        where.userId = userId;
        console.log(`Teacher view - Showing courses for user ID: ${userId}`);
      } else {
        return res.status(403).sendError(res, "Teacher access required for admin view");
      }
    } else {
    ...
    if (view_type === 'Teacher' && statusFilter !== undefined) {
      if (statusFilter === "true" || statusFilter === "active") {
        where.status = "active";
        where.is_active = true;
      } else if (statusFilter === "false" || statusFilter === "inactive") {
...
export const updateCourse = async (req: Request, res: Response) => {
  try {
    ...
    const validModes = ['online', 'offline'];
    if (mode && !validModes.includes(mode)) {
      return res.status(400).sendError(res, "Mode must be one of: online, offline");
    }
    // Price validation for paid courses
    if (priceType === 'paid' && (!price || Number(price) <= 0)) {
